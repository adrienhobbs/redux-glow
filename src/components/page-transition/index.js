...
const PageTransition = React.createClass({
  ...
  leftTransTL (startOrigin) {
    ...
    TL.to(this.refs.transLeft, 1.6, {scaleX: 1, transformOrigin: startOrigin, ease: Expo.easeInOut});
    return TL;
  },
  rightTransTL (startOrigin) {
    const TL = new TimelineLite();
    TL.to(this.refs.transRight, 1.5, {zIndex:12, scaleX: 1, transformOrigin: startOrigin, ease: Expo.easeInOut});
    return TL;
  },
  leftTransTLOut (endOrigin) {
    const TL = new TimelineLite();
    TL.to(this.refs.transLeft, 0.25, {scaleX: 0, transformOrigin: endOrigin, ease: Quart.easeInOut});
    return TL;
  },
  rightTransTLOut (endOrigin) {
    const TL = new TimelineLite();
    TL.to(this.refs.transRight, 0.25, {scaleX: 0, transformOrigin: endOrigin, ease: Quart.easeInOut});
    return TL;
  },
  pageTransTL (pageEl, xStart) {
    const TL = new TimelineLite();
    TL.fromTo(pageEl, 1.8, {xPercent: xStart}, { xPercent: 0, ease: Expo.easeInOut, onComplete: () => {
